...
import requests
from .dcc import DCCHandler, DCCDownload
from .search_parser import SearchResultParser, BookDetail


class IRCSession:
    """Manages a persistent IRC session for downloading multiple files."""
    
    def __init__(self, server: str = "irc.irchighway.net", port: int = 6667, channel: str = "#ebooks"):
        self.server = server
        self.port = port
    ...
    def _generate_random_nickname(self) -> str:
        ...
        adjectives = ["Dark", "Web", "Quick", "Silent", "Swift", "Digital", "Cyber", "Net"]
        ...
        if random.choice([True, False]):
            base += random.choice(["_", ""]) + ''.join(random.choices(string.ascii_lowercase, k=2))
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            print(f"[IRC] Connecting to {self.server}:{self.port} as {self.nickname}...")
    ...
    def _is_potential_search_result(self, line: str) -> bool:
        ...
        return (line.startswith('!') and 
                any(ext in line.lower() for ext in ['.epub', '.pdf', '.mobi', '.txt', '.zip', '.rar']))
    ...
    def search_books(self, author: str, title: Optional[str] = None) -> List[Dict]:
        ...
        print(f"[IRC] Waiting for search results...")
        ...
        if hasattr(self, '_search_results') and self._search_results:
            books, parse_errors = self.search_parser.parse_search_results(self._search_results)
            
            # Filter results if specific criteria provided
            if author or title:
                filter_term = f"{author} {title}".strip()
                books = self.search_parser.filter_results(books, author_filter=filter_term)
            ...
            print(f"[IRC] No search results received")
            return []
    ...
    def _is_search_result(self, line: str) -> bool:
        ...
        for pattern in patterns:
            if re.search(pattern, line) and any(ext in line.lower() for ext in ['.epub', '.pdf', '.mobi', '.txt', '.zip']):
                return True
        return False
    ...
    def download_file(self, download_command: str, custom_filename: Optional[str] = None) -> Dict:
        """Download a file by sending the download command to IRC."""
        if not self.connected or not self.socket:
            raise Exception("Not connected to IRC")
        ...
        self.socket.send(f"{download_command}\r\n".encode())
        
        # Wait for download link
        download_link = None
        self.socket.settimeout(60)  # 1 minute timeout for download link
        ...
        while time.time() - start_time < 60:
            try:
                ...
                if resp:
                    ...
                    link_match = re.search(r'(https?://\S+)', resp)
                    if link_match:
                        download_link = link_match.group(1)
                        print(f"[IRC] Got download link: {download_link}")
                        break
            except socket.timeout:
                break
            except Exception as e:
        ...
        try:
            ...
            if not custom_filename:
                ...
                url_filename = download_link.split('/')[-1]
                if '.' in url_filename:
                    custom_filename = url_filename
                else:
                    custom_filename = f"download_{int(time.time())}"
            
            file_path = os.path.join(self.download_dir, custom_filename)
            
            print(f"[IRC] Downloading to: {file_path}")
                    ...
                    if chunk:
                        ...
                        if total_size > 0:
                            ...
                            if downloaded % (1024 * 1024) == 0:  # Log every MB
                                print(f"[IRC] Download progress: {progress:.1f}%")
            
            print(f"[IRC] Download completed: {file_path}")
    ...
    def _extract_zip(self, zip_path: str) -> List[str]:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                ...
                extracted_files = [os.path.join(extract_dir, name) for name in zip_file.namelist()]
...
def search_and_download(session_id: str, author: str, title: Optional[str] = None) -> Dict:
...
def download_from_result(session_id: str, download_command: str, filename: Optional[str] = None) -> Dict:
...
def start_irc_search(author: str) -> str:
    """Legacy function - creates a session and starts a search."""
    ...
    def search_thread():
        ...
        result = search_and_download(session_id, author)
        print(f"[IRC] Legacy search result for {author}: {result}")
...
def get_search_status(search_id: str) -> dict:
    """Legacy function - get session status."""
    status = get_session_status(search_id)
    if status["success"]:
